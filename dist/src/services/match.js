...
function buildCtx(p) {
    ...
    const statesSet = new Set([normState(p.state)]);
    for (const s of p.otherStates ?? [])
        statesSet.add(normState(s));
    for (const s of p.remoteEmployeeStates ?? [])
        statesSet.add(normState(s));
    for (const s of p.salesStates ?? [])
        statesSet.add(normState(s));
    if (p.collectsFromCA)
        statesSet.add("CA");
    for (const s of p.collectsFromOtherStates ?? [])
        statesSet.add(normState(s));
...
function evalConditions(conditions, ctx) {
    ...
    if (typeof conditions.employeesMin === "number") {
        ...
        reasons.push(`Meets minimum employees threshold (have ${ctx.employeesTotal} ≥ ${conditions.employeesMin}).`);
    }
    if (typeof conditions.employeesMax === "number") {
        if ((ctx.employeesTotal ?? 0) > conditions.employeesMax)
            return { ok: false, reasons: [] };
        reasons.push(`Within maximum employees threshold (have ${ctx.employeesTotal} ≤ ${conditions.employeesMax}).`);
    }
    // 2) industry (case-insensitive)
    if (Array.isArray(conditions.industry) && conditions.industry.length) {
        const set = new Set(conditions.industry.map(lc));
        if (!set.has(ctx.industryLc))
            return { ok: false, reasons: [] };
    ...
    if (Array.isArray(conditions.alcoholType) && conditions.alcoholType.length) {
        if (!ctx.alcoholType ||
            !conditions.alcoholType.includes(ctx.alcoholType))
            return { ok: false, reasons: [] };
        reasons.push(`Alcohol type allowed (${ctx.alcoholType}).`);
    }
    if (Array.isArray(conditions.alcoholSalesContext) &&
        conditions.alcoholSalesContext.length) {
        if (!ctx.alcoholSalesContext ||
            !conditions.alcoholSalesContext.includes(ctx.alcoholSalesContext))
            return { ok: false, reasons: [] };
    ...
    if (Array.isArray(conditions.cities) && conditions.cities.length) {
        const allowed = conditions.cities.map(normCity);
        if (!hasAny(allowed, ctx.citiesArr))
            return { ok: false, reasons: [] };
    ...
    if (typeof conditions.employsMinors === "boolean") {
        if (ctx.employsMinors !== conditions.employsMinors)
            return { ok: false, reasons: [] };
        reasons.push(ctx.employsMinors ? "Employs minors." : "Does not employ minors.");
    }
    if (Array.isArray(conditions.minorsAges) && conditions.minorsAges.length) {
        if (!hasAny(conditions.minorsAges, ctx.minorsAges))
            return { ok: false, reasons: [] };
    ...
    if (typeof conditions.tippedWorkers === "boolean") {
        if (ctx.tippedWorkers !== conditions.tippedWorkers)
            return { ok: false, reasons: [] };
        reasons.push(ctx.tippedWorkers ? "Has tipped workers." : "No tipped workers.");
    }
    if (Array.isArray(conditions.tippedPercentBand) &&
        conditions.tippedPercentBand.length) {
        if (!ctx.tippedPercentBand ||
            !conditions.tippedPercentBand.includes(ctx.tippedPercentBand))
            return { ok: false, reasons: [] };
    ...
    if (Array.isArray(conditions.collectsFromOtherStates) &&
        conditions.collectsFromOtherStates.length) {
        const want = new Set(conditions.collectsFromOtherStates.map(normState));
        if (!ctx.collectsFromOtherStates.some((s) => want.has(s)))
            return { ok: false, reasons: [] };
        reasons.push("Collects data from specified states.");
    }
    if (Array.isArray(conditions.dataVolumeBand) &&
        conditions.dataVolumeBand.length) {
        if (!ctx.dataVolumeBand ||
            !conditions.dataVolumeBand.includes(ctx.dataVolumeBand))
            return { ok: false, reasons: [] };
...
function computeDueDate(conditions, payload) {
    const dueFrom = conditions?.dueFrom; // e.g., "firstPayrollDate"
    const dueInDays = Number(conditions?.dueInDays ?? 0);
    if (!dueFrom)
        return null;
    const base = payload[dueFrom]; // "YYYY-MM-DD"
    if (!base)
        return null;
...
function toPublic(r, reasons, payload) {
...
function cacheKey(states, cities) {
...
export async function matchRules(payload) {
    ...
    if (!rows) {
       ...
       ORDER BY
         ...
         title ASC`, [ctx.statesArr, ctx.citiesArr]);
        rows = r.rows;
    ...
    for (const r of rows) {
        ...
        if (ok)
            matched.push({ row: r, reasons });
    }
    if (process.env.DEBUG_MATCH) {
        const ms = Number((process.hrtime.bigint() - t1) / 1000000n);
    ...
    const count = grouped.federal.length +
