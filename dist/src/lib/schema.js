...
/** ======= Reference enums ======= */
export const LocationTypes = ["store", "office", "warehouse", "home"];
export const RevenueBands = ["<100k", "100k-1M", "1M-5M", ">=5M"];
...
export const AlcoholTypes = ["beer", "wine", "spirits"];
...
export const DataVolumeBands = ["<50k", "50k-100k", ">=100k"];
/** Nuance enums */
...
export const MinorsAges = ["<16", "16-17"];
export const TippedPercentBand = ["<20%", "20-50%", ">50%"];
export const NumLocations = ["1", "2-4", "5+"];
/** ======= Helpers ======= */
...
const optionalDateStr = z
    .preprocess((v) => (v === "" || v == null ? undefined : v), z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Use YYYY-MM-DD"))
    .optional();
...
/** NAICS inputs */
const naicsCode = z
    .preprocess((v) => (v === "" || v == null ? undefined : String(v).trim()), z.string().regex(/^\d{2,6}$/, "2–6 digit NAICS"))
    .optional();
const naicsTitle = optionalTrimmed;
/** ZIP (optional but validated if present) */
const zipOptional = z
    .preprocess((v) => (v === "" || v == null ? undefined : String(v).trim()), z.string().regex(/^\d{5}(-\d{4})?$/, "Use 12345 or 12345-6789"))
    .optional();
...
export const BusinessFormSchema = z
    ...
    .refine((v) => !v.commercialVehicles || typeof v.hasCDLDrivers === "boolean", {
    path: ["hasCDLDrivers"],
