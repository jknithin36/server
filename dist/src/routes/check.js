...
check.post("/v1/check", async (req, res) => {
    ...
    try {
        ...
        const stripReasons = (arr) => includeReasons
            ? arr
            : arr.map(({ appliesBecause, ...rest }) => rest);
        const capped = (arr) => typeof perBucketLimit === "number" ? arr.slice(0, perBucketLimit) : arr;
        const response = {
            business: {
        ...
        console.log("POST /v1/check", JSON.stringify({
